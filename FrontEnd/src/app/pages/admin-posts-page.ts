  ...
  template: `
    ...
    @if (deleting(); as post) {
      <section class="panel mt-6 border-red-200" role="alert" aria-labelledby="delete-post-title">
        ...
        <p class="my-4">The post and its applications will be hidden.</p>
    ...
    <section class="panel mt-6">
      <div class="mb-5 flex flex-wrap gap-3">
        <label class="grow"><span class="sr-only">Search posts</span><input placeholder="Search title or founder" [ngModel]="search()" (ngModelChange)="search.set($event)"></label>
        <label><span class="sr-only">Sort posts</span><select name="sortOrder" [(ngModel)]="sortOrder" (ngModelChange)="load()" [disabled]="loading() || busy()" aria-label="Sort posts">
      ...
      @else {
        <p class="help mb-3">{{ filtered().length }} posts</p>
...
export class AdminPostsPage {
  ...
  async load() {
    ...
    try {
      const [posts, users] = await Promise.all([this.service.getPosts(this.sortOrder), this.service.getUsers()]);
      this.posts.set(posts); this.users.set(users);
  ...
  async save() {
    ...
    try {
      ...
      this.editing.set(false); this.success.set('Post saved.'); await this.load();
    } catch (error) { this.error.set(errorMessage(error)); }
  ...
  async confirmDelete() {
    ...
    try {
      ...
      this.deleting.set(null); this.success.set('Post deleted.'); await this.load();
    } catch (error) { this.error.set(errorMessage(error)); }
    finally { this.busy.set(false); }
  }
}
